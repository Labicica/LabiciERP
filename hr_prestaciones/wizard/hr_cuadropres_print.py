...
class hr_cuadropres_printwizard(osv.osv_memory):

    """
    This wizard will print the islr reports for a given partner.
    """

    _name = 'hr.cuadropres.print'
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        context = context or {}
        slip_pool = self.pool.get('hr.cuadropres.print')
        data  = {}
        slip_ids = []
        ...
        cr.execute("SELECT a.id FROM hr_employee a INNER JOIN hr_cuadropres b ON (b.name=a.id) WHERE b.meses>='%s' and b.meses<='%s' and b.ddgarantias=15 GROUP BY a.id ORDER BY a.passport_id" % (from_date,to_date))
        res = cr.fetchall()
        if res:
            print res
            for r in res:
                ...
                WHERE name=%s and meses<='%s' ORDER BY meses desc LIMIT 12''' % (idemp ,to_date)
                cr.execute(sql)
                garantias = cr.fetchall()
                for emp in garantias:
                    ini_date=emp[1]          

                sql = '''SELECT name, sum(ddgarantias) as ddgarantias,sum(dagarantias) as dagarantias,
                sum(dgprestaciones) as dgprestaciones,
                sum(apsociales) as apsociales FROM hr_cuadropres 
                WHERE name=%s and meses<'%s' GROUP BY name ORDER BY name''' % (idemp ,ini_date)
                cr.execute(sql)
                garantias = cr.fetchall()
                for emp in garantias:
                    reg = {
                        ...
                        'ddgarantias':emp[1],
                        'dagarantias':emp[2],
                        'dgprestaciones':emp[3],          
                        'apsociales':emp[4], 
                        'dgpacumuladas':0, 
                    }                 
                    print slip_ids.append(slip_pool.create(cr, uid, reg, context=context))
                ...
                WHERE name=%s and meses>='%s' and meses<='%s' ORDER BY meses''' % (idemp ,ini_date, to_date)
        ...
        return {
            ...
            'report_name': 'hr.cuadropres.garantias', 'datas': data}
