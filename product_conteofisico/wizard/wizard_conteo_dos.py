...
class cf_conteo_dos(osv.osv_memory):
    ...
    def conteo_add(self, cr, uid, ids, context=None):
        ...
        data = self.pool.get('cf.conteo.dos').read(cr, uid, ids)[0]
        conteo_obj = self.pool.get('cf.conteo.move')    
        conteo_seek = conteo_obj.search(cr, uid, [('ubica_id', '=', data['ubicacion'][0] )])       
        conteo_obj.write(cr, uid, conteo_seek, {'conteo_2': data['cantidad']}, context=context)
        
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'cf.conteo.dos',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': 0,
            'views': [(False, 'form')],
