...
class hr_cuadropres_printwizard(osv.osv_memory):
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        cr.execute("SELECT a.id FROM hr_employee a INNER JOIN hr_cuadropres b ON (b.name=a.id) WHERE b.meses>='%s' and b.meses<='%s' and b.ddgarantias=15 ORDER BY a.passport_id" % (from_date,to_date))
        res = cr.fetchall()
        if res:
            print res
            for r in res:
